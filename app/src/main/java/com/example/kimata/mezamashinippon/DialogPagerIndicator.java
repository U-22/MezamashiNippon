...
public class DialogPagerIndicator extends RadioGroup {
    public DialogPagerIndicator(Context context) {
        this(context, null);
    }

    public DialogPagerIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    public void setCount(int count) {
        ...
        for (int i = 0; i < count; i++) {
            // RadioButtonにインジケーターの画像をセット
            RadioButton rb = new RadioButton(getContext());
            rb.setFocusable(false);
            rb.setClickable(false);
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.JELLY_BEAN_MR1) {
                // 4.2以前は背景画像でpaddingを設定していたため、ラジオボタンのサイズをセットしなければならない
